...
const Laptops = () => {
    ...
    const [laptopBrands] = useState(null);
    ...
    useEffect(() => {
        fetchLaptops();
    }, [laptopBrands, minPrice, maxPrice]);

    const fetchLaptops = async () => {
        try {
            const data = await ProductApiService.getAllPageProductByCategoryAndBrand(1, 10, laptopCategories, laptopBrands, minPrice, maxPrice);
    ...
    const handleViewAllClick = () => {
        navigate(ROUTES.PAGE_LAPTOP, { state: {filteredProducts: laptops , products: laptops, brands: brands} });
    };
    ...
    const handleBrandClick = (brandName, brandId) => {
        navigate(ROUTES.PAGE_LAPTOP, { state: { brandName: brandName, brandId: brandId } });
    }
        ...
        <div className="grid grid-cols-12 gap-4 pt-5 border-t-2 border-gray-300">
            <div className="col-start-2 col-span-10 flex flex-col mb-20 gap-2">
                <div className="flex flex-row justify-between items-center w-full">
                    <h2 className="text-2xl font-bold text-blue-500">Laptop nổi bật</h2>
