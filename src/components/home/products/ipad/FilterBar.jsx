...
const FilterBar = ({products, brands, brandName, onFilter, onBrandSelect}) => {
    ...
    const handleApplyFilters = () => {
        ...
        if (selectedNeed) {
            switch (selectedNeed) {
                case 'Văn phòng':
                    filteredProducts = filteredProducts.filter(product => product.category === 'Office Laptop');
                    break;
                case 'Gaming':
                    filteredProducts = filteredProducts.filter(product => product.category === 'Gaming Laptop');
                    break;
                case 'Đồ họa Kỹ thuật':
                    filteredProducts = filteredProducts.filter(product => product.category === 'Graphic Design Laptop');
                    break;  
                case 'Cao cấp Sang trọng':
                    filteredProducts = filteredProducts.filter(product => product.price >= 30000000);
                    break;
                case 'Ultrabook':
                    filteredProducts = filteredProducts.filter(product => product.category === 'Ultrabook');
                    break;
            }
        }
            ...
            <div className="flex flex-wrap justify-start mb-4">
                <div className='flex flex-col'>
                    ...
                    <div className='flex flex-wrap'> 
                        {['Văn phòng', 'Gaming', 'Đồ họa Kỹ thuật', 'Cao cấp Sang trọng', 'Ultrabook'].map((need) => (
                            <button key={need} 
