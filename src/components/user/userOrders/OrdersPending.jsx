...
function OrdersPending() {
    ...
    const fetchMyOrders = async () => {
        try {
            ...
            const filteredOrders = response.content.filter(order => order.orderStatus === 'PENDING');
            ...
            <div className="col-start-2 col-span-8 grid grid-cols-4 gap-4 pb-10">
                ...
                <div className="col-span-3 flex flex-col justify-start pr-8">
                    <div className="grid grid-cols-6 border border-gray-300 rounded-t-lg pt-2 mb-4">
                        ...
                        <button onClick={() => navigate(ROUTES.USER_ORDERS_PENDING)} className="text-blue-500 font-semibold pb-2 border-b-2 border-blue-500">Chờ xác nhận</button>
                        <button onClick={() => navigate(ROUTES.USER_ORDERS_CONFIRMED)} className="text-gray-600 font-semibold pb-2 border-b-2">Chờ lấy hàng</button>
                        ...
                        <button onClick={() => navigate(ROUTES.USER_ORDERS_CANCELLED)} className="text-gray-600 font-semibold pb-2 border-b-2">Đã hủy</button>
                    </div>
                    <div className="">
